...
if(touch) {
    ...
    let inputRight = (input, chunks) => {
        if(chunks.right.length > 0) {
            ...
            if(!chunks.selection) {
                ...
                input.selectionStart = input.selectionEnd = chunks.left.length + chunks.selection.length;
            }
        } else {
            if(chunks.selection) {
                input.selectionStart = input.selectionEnd = chunks.left.length + chunks.selection.length + chunks.right.length;
            }
        }
    };
    ...
    let allowKeys = ['Tab','ShiftLeft','ShiftRight','ControlLeft','ControlRight','ArrowLeft','ArrowRight','End','Home','Delete','Backspace'];
